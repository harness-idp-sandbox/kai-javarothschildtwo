...
# run app
java -jar target/javarothschildtwo-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/javarothschildtwo:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/javarothschildtwo -n kaiser

# get external endpoint
kubectl get svc javarothschildtwo-svc -n kaiser
```
...
```bash
kubectl port-forward svc/javarothschildtwo-svc 8080:80 -n kaiser
...
kubectl rollout restart deploy/javarothschildtwo -n kaiser
kubectl rollout status deploy/javarothschildtwo -n kaiser
```
...
info.app.name=javarothschildtwo
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/javarothschildtwo/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javarothschildtwo/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
